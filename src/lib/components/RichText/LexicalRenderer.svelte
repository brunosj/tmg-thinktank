<script lang="ts">
	...
	function getTextFormatClasses(format?: number | string): string {
		...
		if (format & 1) classes.push('font-bold'); // Bold
...
{#snippet renderNode(node: LexicalNode)}
	...
	{:else if node.type === 'heading'}
		{#if node.tag === 'h1'}
			<h1 class="mb-6 mt-8 text-4xl font-bold">
				{#if node.children}
					{#each node.children as child}
						{@render renderNode(child)}
					{/each}
		...
		{:else if node.tag === 'h2'}
			<h2 class="mb-5 mt-7 text-3xl font-bold">
				{#if node.children}
					{#each node.children as child}
						{@render renderNode(child)}
					{/each}
		...
		{:else if node.tag === 'h3'}
			<h3 class="mb-4 mt-6 text-2xl font-bold">
				{#if node.children}
					{#each node.children as child}
						{@render renderNode(child)}
					{/each}
		...
		{:else if node.tag === 'h4'}
			<h4 class="mb-3 mt-5 text-xl font-bold">
				{#if node.children}
					{#each node.children as child}
						{@render renderNode(child)}
					{/each}
		...
		{:else if node.tag === 'h5'}
			<h5 class="mb-2 mt-4 text-lg font-bold">
				{#if node.children}
					{#each node.children as child}
						{@render renderNode(child)}
					{/each}
		...
		{:else if node.tag === 'h6'}
			<h6 class="mb-2 mt-3 text-base font-bold">
				{#if node.children}
					{#each node.children as child}
						{@render renderNode(child)}
					{/each}
